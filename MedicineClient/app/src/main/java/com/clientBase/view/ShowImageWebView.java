...
public class ShowImageWebView extends WebView {
	...
	public void setImageClickListner() {
		...
		this.loadUrl("javascript:(function(){" + "var objs = document.getElementsByTagName(\"img\"); " + "for(var i=0;i<objs.length;i++)  " + "{"
				+ "    objs[i].onclick=function()  " + "    {  " + "        window.imageListener.startShowImageActivity(this.src);  " + "    }  "
				+ "}" + "})()");
	}
	...
	private class MyJavascriptInterface {
		...
		/**
		 * 点击图片启动新的 ShowImageFromWebActivity，并传入点击图片对应的 url 和页面所有图片 对应的 url
		 *
		 * @param url
		 *            点击图片对应的 url
		 */
...
//            intent.putExtra(Consts.IMAGE_URL, url);
//            intent.putStringArrayListExtra(Consts.IMAGE_URL_ALL, (ArrayList<String>) listImgSrc);
//            intent.setClass(context, ShowImageFromWebActivity.class);
//            context.startActivity(intent);

			for(int i=-0;i<listImgSrc.size();i++){
				Log.i("pony_log", listImgSrc.get(i));
			}
	...
	/***
	 * 获取页面所有图片对应的地址对象， 例如 <img
	 * src="http://sc1.hao123img.com/data/f44d0aab7bc35b8767de3c48706d429e" />
	 *
	 * @param html
	 *            WebView 加载的 html 文本
	 * @return
	...
	/***
	 * 从图片对应的地址对象中解析出 src 标签对应的内容,即 url 例如
	 * "http://sc1.hao123img.com/data/f44d0aab7bc35b8767de3c48706d429e"
	 *
	 * @param listImageUrl
	 *            图片地址对象， 例如 <img
	 *            src="http://sc1.hao123img.com/data/f44d0aab7bc35b8767de3c48706d429e"
	...
	private void downloadImage(String url) {
		// ImageLoaderUtils.downLoadImage(url,Environment.getExternalStorageDirectory().getAbsolutePath()
		// + "/ImagesFromWebView",context);
	}
